...
重构后的 PacToolkits 将业务逻辑从 Avalonia UI 中逐步抽到 `packages/`，形成清晰依赖方向。
...
- Core → 任意 IO（数据库、文件系统、日志框架、配置、UI）
...
- **桌面专属**服务：Toast、Dialog、更新流程、剪贴板、UI 行为等（`Services/Application`、`Services/Infrastructure`）
...
`ISensitiveOperationUnlockService` 定义在 Application 层；实现在桌面 `SensitiveOperationUnlockService`（依赖 Dialog / Toast 等 UI 能力）。

## 演进约束

1. 新业务能力优先落在 Application（接口 + 服务），Infrastructure 补实现
2. Agent 相关共享类型进 `agent-contracts`，避免 UI 与 AHK 各写一份 JSON 模型
