...
public sealed class InventoryOverviewRepo : IInventoryOverviewRepo
{
    ...
    public Task<StockReassignPreviewDto> PreviewStockReassignByTraceCodeAsync(
        string traceCode,
        string targetDrugId,
        string targetSpec,
        CancellationToken ct)
        => _db.WithConnection(async (conn, token) =>
        {
            ...
            string? currentDrugId = null;
            string? currentSpec = null;

            await using (var currentCmd = conn.CreateCommand(currentSql, _opt.CommandTimeoutSeconds))
            {
                currentCmd.AddParam("trace_code", trace);
                await using var reader = await currentCmd.ExecuteReaderAsync(token);
                if (await reader.ReadAsync(token))
                {
                    currentDrugId = reader.GetString(0);
                    currentSpec = reader.GetString(1);
                }
            }
            ...
            {
                countCmd.AddParam("trace_code", trace);
            ...
            const string sampleSql = """
                select trace_code, drug_id, spec, qty, remain
                from trace_pool
                where trace_code = @trace_code
                order by id desc
                limit 20
            ...
            {
                sampleCmd.AddParam("trace_code", trace);
            ...
            var noop = string.Equals(currentDrugId, targetDrug, StringComparison.Ordinal)
                       && string.Equals(currentSpec, targetSpecSafe, StringComparison.Ordinal);

            return new StockReassignPreviewDto(
                TargetExists: targetExists,
                MatchCount: matchCount,
                WillChangeCount: willChangeCount,
                CurrentDrugId: currentDrugId,
